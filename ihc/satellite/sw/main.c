...
void GPIO_Configuration(void)
{
	...
	/* Put the outputs off */
	GPIO_WriteBit(GPIO_Pin_0 | GPIO_Pin_1 | GPIO_Pin_2 | GPIO_Pin_3, Bit_RESET);
...
void HDC2080Init(void)
{
	 ...
	 * */
	i2c_data.tx_buff[1] = 0xF0;	//temperature offset: +- 0.16'C / bit
	I2C_Write_Reg(&i2c_data);
...
void IhcMain(void)
{
	...
	{
		if(Bit_RESET != GPIO_ReadBit(GPIO_Pin_6))	/* HDC2080 measurement ready? */
		{
			HDC2080Read();
			AdcRead();
